...
export class ListComponent implements OnInit {
  ...
  mergedCars$ = combineLatest([this.dataService.getNewCars$(), this.dataService.getUsedCars$()]).pipe(
    tap(([newCars, usedCars]) => console.log(newCars, usedCars)),
    map(([newCars, usedCars]) => {
      const newCarsUsed = newCars.map((car: any) => ({ ...car, isUsed: false }));
      const usedCarsUsed = usedCars.map((car: any) => ({ ...car, isUsed: true }));
